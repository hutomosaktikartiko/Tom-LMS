...
export async function createStripeCheckout(courseId: string, userId: string) {
  try {
    ...
    await createEnrollment({
      studentId: user._id,
      courseId: course._id,
      paymentId: "dunny_test_payment_id",
      amount: course.price,
    });

    return { url: `/courses/${course.slug?.current}` };

    // TODO: 3. Create and configure Stripe Checkout Session with course details
    ...
    // TODO:4. Return checkout session URL for client redirect
